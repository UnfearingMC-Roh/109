...
public class MapleShop {
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) { //최대 메소 21억값 설정
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            if (GameConstants.isThrowingStar(item.getItemId()) || GameConstants.isBullet(item.getItemId())) {
                ...
                price = ii.getPrice(item.getItemId());
                        ...
                        if ((long) (price * quantity + c.getPlayer().getMeso()) >= 2147483647L) {
                c.getSession().write(MaplePacketCreator.confirmShopTransaction((byte) 0));
                c.getPlayer().dropMessage(1, "메소가 21억을 넘을 수 없습니다.");
                return;
            }
